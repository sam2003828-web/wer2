...
class Car:
    ...
    def get_descriptive_name(self):#這裡定義了一個方法get_descriptive_name用來返回車的描述性名稱
        long_name = f"{self.year} {self.make} {self.model}"
        return long_name.title()
...
my_new_car.odometer_reading = 23#這裡直接修改了odometer_reading這個屬性的值
my_new_car.read_odometer()
class Car:
    def __init__(self, make, model, year):
        self.make = make
    ...
    def update_odometer(self, mileage):#這裡定義了一個方法update_odometer用來更新odometer_reading這個屬性的值
        if mileage >= self.odometer_reading:#這裡加了一個條件如果mileage小於odometer_reading就不更新odometer_reading的值
...
class Car:
    ...
    def increment_odometer(self, miles):
        if miles < 0:
            ...
            self.odometer_reading += miles
            print(f"odometer incremented by {miles} miles. Total is now {self.odometer_reading} miles.")
class ElectricCar(Car):#ElectricCar繼承了Car這個類別
    """Represent aspects of a car, specific to electric vehicles."""
    def __init__(self, make, model, year, battery_size):
        """Initialize attributes of the parent class."""
        super().__init__(make, model, year) # 先繼承 Car 的屬性
        self.battery_size = battery_size # 再加入 ElectricCar 自己的新屬性
    def get_descriptive_name(self):#overriding method 覆寫方法
        """Return a neatly formatted descriptive name."""
        long_name = f"{self.year} {self.make} {self.model} with a {self.battery_size}-kWh battery"
        return long_name.title()
...
#class car,battery inheritance electriccar
class Car:
    def __init__(self, make, model, year):
        self.make = make
        self.model = model
...
class ElectricCar(Car):
    ...
    def __init__(self, make, model, year):
        ...
        self.battery = Battery()  # 在 ElectricCar 中創建一個 Battery 實例
    def get_descriptive_name(self):
        """Return a neatly formatted descriptive name."""
...
class Battery:
    ...
    def __init__(self, battery_size=75, show_battery=True):
        """Initialize the battery's attributes."""
        self.battery_size = battery_size  # 真正拿來計算續航力的容量
        self.show_battery = show_battery  # 控制要不要把容量顯示在文字中

    def describe_battery(self):
        """Print a statement describing the battery size."""
        # 如果建立 ElectricCar 時沒有手動給 battery_size，就不要顯示容量文字
...
class ElectricCar(Car):
    """Represent aspects of a car, specific to electric vehicles."""
    def __init__(self, make, model, year, battery_size=None):# 這裡的battery_size是用來給Battery這個類別的__init__方法的參數
        """Initialize attributes of the parent class."""
        super().__init__(make, model, year)
        # 有手動輸入 battery_size 才顯示容量；沒輸入就只用預設值做內部計算
        show_battery = battery_size is not None
        actual_battery_size = 75 if battery_size is None else battery_size
        self.battery = Battery(actual_battery_size, show_battery)  # 建立一個 Battery 物件並存到這台車裡
    def get_descriptive_name(self):
        """Return a neatly formatted descriptive name."""
        long_name = f"{self.year} {self.make} {self.model}"
        # 只有使用者真的有給電池容量時，才把容量加到車名描述裡
...
my_tesla.battery.get_range()
#=========================================
from pizza import Car1
my_car = Car1('audi', 'a4', 2020)
...
my_car.read_odometer()
my_car.update_odometer(int(input("enter the mileage you want to update: ")))
my_car.read_odometer()
#======================================================================
from pizza import ElectricCar
my_tesla = ElectricCar('tesla', 'model s', 2020, 100)
print(my_tesla.get_descriptive_name())
my_tesla.battery.describe_battery()
my_tesla.battery.get_range()
#improting multiple classes 從模組裡面匯入多個類別
from pizza import Car1, ElectricCar
my_car = Car1('audi', 'a4', 2020)
print(my_car.get_descriptive_name())
my_tesla = ElectricCar('tesla', 'model s', 2020)
print(my_tesla.get_descriptive_name())
#importing an entire module 從模組裡面匯入整個模組
import pizza#這個方法比較好因為這樣就不會有命名衝突的問題如果你從模組裡面匯入
#多個類別的話可能會有命名衝突的問題
my_car = pizza.Car1('audi', 'a4', 2020)
print(my_car.get_descriptive_name())
my_tesla = pizza.ElectricCar('tesla', 'model s', 2020, 100)
print(my_tesla.get_descriptive_name())
#importin all classes from a module 從模組裡面匯入所有的類別
from pizza import *
my_car = Car1('audi', 'a4', 2020)
print(my_car.get_descriptive_name())
my_tesla = ElectricCar('tesla', 'model s', 2020, 100)
print(my_tesla.get_descriptive_name())
#importing a module into a module 從模組裡面匯入模組
from pizza import Car1
